...
MenuSetRouter.get('/menu_setup', async (req, res) => {
    let id = req.query.id,
        ...
        venue_whr = req.query.venue_id ? `AND a.srv_res_id = "${req.query.venue_id}"` : '';
    ...
    var oth_dt = await F_Select(oth_sql),
        logo_sql = `SELECT a.hotel_id, a.srv_res_flag, a.srv_res_id, a.logo_url, a.logo_path FROM td_logo a WHERE a.hotel_id = "${id}" ${venue_whr}`,
...
MenuSetRouter.get('/get_menu_img', async (req, res) => {
    var data = req.query;
    var select = "*",
...
MenuSetRouter.get('/aboutus', async (req, res) => {
    ...
    menu_id = req.query.menu_id,
    flag = req.query.flag;
    let sql = `SELECT * FROM td_about WHERE hotel_id = ${hotel_id} AND srv_res_flag = '${flag}' AND restaurant_id = "${res_id}" AND menu_id = '${menu_id}'`;
    var data = await F_Select(sql);
    res.send(data);
})
...
MenuSetRouter.get("/items", async (req, res) => {
	var hotel_id = req.query.hotel_id,
		...
		menu_name = menu_name = menu_id == 'B' ? 'Breakfast' : (menu_id == 'A' ? 'Bar' : (menu_id == 'L' ? 'Lunch' : (menu_id == 'D' ? 'Dinner' : (menu_id == 'R' ? 'Brunch'
         : (menu_id == 'S' ? 'Special' : (menu_id == 'V' ? 'Menu Service' : ''))))));
	let whr = sec_id > 0 ? `AND a.section_id = "${sec_id}"` : "";
	let sql = `SELECT a.*, '${menu_name}' menu_name, '${menu_id}' menu_id FROM md_service_items a WHERE a.hotel_id = ${hotel_id} ${whr}`;
	var data = await F_Select(sql);
	res.send(data);
});
...
MenuSetRouter.get("/item_price", async (req, res) => {
  var item_id = req.query.item_id,
    menu_id = req.query.menu_id,
    menu_name = menu_id == 'B' ? 'Breakfast' : (menu_id == 'A' ? 'Bar' : (menu_id == 'L' ? 'Lunch' : (menu_id == 'D' ? 'Dinner' : (menu_id == 'R' ? 'Brunch'
    : (menu_id == 'S' ? 'Special' : (menu_id == 'V' ? 'Menu Service' : ''))))));
  let sql = `SELECT a.*, b.item_name, b.section_id, '${menu_name}' menu_name, '${menu_id}' menu_id FROM md_service_item_description a, md_service_items b WHERE a.service_item_id=b.service_item_id AND a.service_item_id =${item_id}`;
  var data = await F_Select(sql);
  res.send(data);
});
...
MenuSetRouter.get('/res_dtls', async (req, res) => {
    var res_id = req.query.id;
    let whr = res_id > 0 ? `WHERE a.id = "${res_id}" AND b.approval_flag = 'A'` : `WHERE b.approval_flag = 'A'`;
...
MenuSetRouter.get('/res_dtls_custom', async (req, res) => {
    ...
    let whr = res_id > 0 ? `WHERE a.id = "${res_id}" AND active_user = 'Y'` : (res_id == 0 ? `WHERE active_user = 'Y'` :(flag != 'H' && flag ? `WHERE active_user = 'Y' AND account_type = '${flag}' AND sales_agent = ${sales_id}` : `WHERE active_user = 'Y' AND account_type = '${flag}'`))
    // let whr =
    //     res_id > 0
...
MenuSetRouter.get("/get_url", async (req, res) => {
  var data = req.query;
  var select =
      "a.*, IF(a.srv_res_flag = 'R', (SELECT b.name FROM td_quest_res_bars b WHERE a.srv_res_id=b.id AND a.hotel_id=b.hotel_id), (SELECT c.service_name FROM td_quest_service c WHERE a.srv_res_id=c.id AND a.hotel_id=c.hotel_id)) res_srv_name",
    table_name = "md_url a",
    whr = `a.hotel_id = ${data.id} ${
      data.srv_res_flag
        ? 'AND a.srv_res_flag = "' + data.srv_res_flag + '"'
        : ""
    } ${data.srv_res_id ? 'AND a.srv_res_id = "' + data.srv_res_id + '"' : ""}`,
    order = null;
  var res_dt = await db_Select(select, table_name, whr, order);
  res.send(res_dt);
});
