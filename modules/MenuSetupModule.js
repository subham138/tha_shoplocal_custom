...
const AboutUsSave = async (data) => {
  ...
  var chk_dt = await Check_Data(
    (db_name = "td_about"),
    (whr = `WHERE  hotel_id = ${data.hotel_id} AND srv_res_flag = "${data.srv_res_flag}" AND restaurant_id = "${data.restaurant_id}" AND menu_id = "${data.menu_id}"`)
  );
  var sql = "";
  if (chk_dt > 1) {
    sql = `INSERT INTO td_about (hotel_id, srv_res_flag, restaurant_id, menu_id, about_us, created_by, created_dt) VALUES 
        ("${data.hotel_id}", "${data.srv_res_flag}", "${data.restaurant_id}", "${data.menu_id}", "${data.aboutus}", "${data.restaurant_id}", "${datetime}")`;
  } else if (chk_dt == 1) {
    sql = `UPDATE td_about SET about_us = "${data.aboutus}", modified_by = "${data.hotel_id}", modified_dt = "${datetime}" WHERE hotel_id = ${data.hotel_id} AND srv_res_flag = "${data.srv_res_flag}" AND restaurant_id = "${data.restaurant_id}" AND menu_id = "${data.menu_id}"`;
  }
